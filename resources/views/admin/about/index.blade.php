    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-tools">
                <a href="{{ route('admin.about.create') }}" class="btn btn-success btn-sm">Tambah Data {{ $title }}</a>
            </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><img src="{{ Storage::disk('local')->url($v->thumbnail) }}" alt="Thumbnail" width="200px"></td>
                        <td>{{ $v->updated_at ? Carbon\Carbon::parse($v->updated_at)->diffForHumans() : Carbon\Carbon::parse($v->created_at)->diffForHumans() }}</td>
                        <td class="text-center">
                            <form action="{{ route('admin.about.destroy', $v->id) }}" method="POST">
                                @method('DELETE')
                                @csrf
                                {{-- <a class="btn btn-primary btn-block btn-sm" href="{{ route('admin.about.show', $v->id) }}">
                                    <i class="fas fa-folder mr-2"></i>View
                                </a> --}}
                                <a class="btn btn-info btn-block btn-sm" href="{{ route('admin.about.edit', $v->id) }}">
